...
import reco.frame.tv.bitmap.LoaderImpl;
...
public class TvBitmap {
	...
	public void displayHD(final View view, final String url) {
		...
		if (bitmap != null) {
			...
			bitmapLoadAndDisplayExecutor.submit(new Runnable() {
				@Override
				public void run() {
					
					final Bitmap bitmapDisk=impl.getBitmapFromFile(url);
					if (bitmapDisk!=null) {
						...
						final Bitmap bitmap = impl.getBitmapFromUrl(url, true);
						handler.post(new Runnable() {
							@Override
							public void run() {
								if (bitmap != null) {
